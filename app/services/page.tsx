  ...
  return (
    <div className="flex flex-col min-h-screen">
      ...
      <main className="flex-grow bg-white">
        {/* Hero Section */}
        <section className="relative py-20 overflow-hidden bg-gradient-to-b from-purple-50/50 to-white">
          <div className="absolute top-0 left-0 w-full h-full -z-10 opacity-30">
            <div className="absolute top-10 left-10 w-64 h-64 bg-purple-200 rounded-full blur-3xl animate-pulse" />
            <div className="absolute bottom-10 right-10 w-96 h-96 bg-purple-200 rounded-full blur-3xl animate-pulse" style={{ animationDelay: '2s' }} />
          </div>
              ...
              {categories.map((cat, i) => (
                <motion.div key={i} variants={fadeInUp}>
                  <Link href={`/user/search?service=${cat.name.toLowerCase()}`} className="group block h-full">
                    <Card hover className="h-full flex flex-col p-8 group-hover:border-purple-500 transition-all duration-300 relative overflow-hidden bg-white shadow-soft">
                      {/* Hover background effect */}
                      <div className="absolute top-0 right-0 w-24 h-24 bg-purple-500/5 rounded-bl-full translate-x-12 -translate-y-12 group-hover:translate-x-0 group-hover:-translate-y-0 transition-transform duration-500" />
                      ...
                      <div className="flex items-center text-purple-600 font-bold group">
        ...
        <section className="py-24 bg-gray-50 overflow-hidden relative">
          ...
          {/* Decorative shapes */}
          <div className="absolute top-0 right-0 w-64 h-64 bg-purple-100 rounded-full -translate-y-1/2 translate-x-1/2 blur-3xl opacity-50" />
