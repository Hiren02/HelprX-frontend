                        ...
                        {steps.map((s, index) => {
                            ...
                            return (
                                <div key={s.id} className="flex flex-col items-center gap-2 bg-gray-50/50 px-2">
                                    <div className={`w-10 h-10 rounded-full flex items-center justify-center font-bold text-lg transition-all duration-300 ${isActive || isCompleted
                                            ? 'bg-primary-600 text-white shadow-lg shadow-primary-500/30 scale-110'
                                            : 'bg-white border-2 border-gray-200 text-gray-400'
                                        }`}>
                                        {isCompleted ? <Check className="w-5 h-5" /> : s.number}
                                    </div>
                                    <span className={`text-sm font-medium transition-colors ${isActive || isCompleted ? 'text-primary-700' : 'text-gray-400'
                                        }`}>
                                        {s.label}
                                    </span>
                                </div>
                                            ...
                                            {loadingAddresses ? (
                                                ...
                                                <div className="grid gap-3 max-h-[400px] overflow-y-auto pr-2 custom-scrollbar">
                                                    {addresses.map((address) => (
                                                        <div
                                                            key={address.id}
                                                            className={`border-2 rounded-xl p-4 cursor-pointer transition-all duration-200 relative ${formData.addressId === address.id
                                                        ...
                                                        >
                                                            <div className="flex justify-between items-start">
                                                                <div className="flex items-start gap-3">
                                                                    <div className={`mt-0.5 w-5 h-5 rounded-full border flex items-center justify-center ${formData.addressId === address.id ? 'border-primary-600 bg-primary-600' : 'border-gray-400'
                                                                        }`}>
